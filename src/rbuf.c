...
rbuf_t *rb_create(uint32_t size) {
    ...
    rb->size = size;
    for (i = 0; i < size; i++) { // we need to create size + 1 nodes 
        rbuf_page_t *page = calloc(1, sizeof(rbuf_page_t));
        if (!rb->head) {
...
void *rb_read(rbuf_t *rb) {
    ...
    for (i = 0; i < RBUF_MAX_RETRIES; i++) {
        ...
        if (ATOMIC_CMPXCHG(read_sync, 0, 1)) {
            ...
            if (rb->reader == commit || ATOMIC_READ(commit->next) == head || head == tail)
            { // nothing to read
            ...
            if (ATOMIC_CMPXCHG(rb->reader->prev->next, RBUF_FLAG_ON(head, RBUF_FLAG_HEAD), rb->reader)) {
...
int rb_write(rbuf_t *rb, void *value) {
    ...
    do {
        ...
        if (temp_page == commit || (ATOMIC_READ(temp_page->next) == head)) {
    ...
    int did_update = 0;
    if (rb->overwrite_mode && ATOMIC_CMPXCHG(tail->next, RBUF_FLAG_ON(nextp, RBUF_FLAG_HEAD), RBUF_FLAG_ON(nextp, RBUF_FLAG_UPDATE))) {
        did_update = 1;
    ...
    if (ATOMIC_READ(num_writers) == 1) {
        ATOMIC_CMPXCHG(rb->commit, commit, ATOMIC_READ(rb->tail)->prev);
    }

    if (did_update) {
        ATOMIC_CMPXCHG(tail->next, RBUF_FLAG_ON(nextp, RBUF_FLAG_UPDATE), rb->overwrite_mode ? RBUF_FLAG_OFF(nextp, RBUF_FLAG_UPDATE) : RBUF_FLAG_ON(nextp, RBUF_FLAG_HEAD) );
    }
