...
pqueue_t *
pqueue_create(uint32_t size, pqueue_free_value_callback free_value_cb)
{
...
pqueue_pull_highest(pqueue_t *pq, void **value, int32_t *prio)
{
    ...
    int rc = binheap_delete_maximum(pq->heap, (void **)&item, NULL);
    if (rc == 0) {
...
{
    pqueue_item_t *item = NULL;
    int rc = binheap_delete_minimum(pq->heap, (void **)&item, NULL);
    if (rc == 0) {
